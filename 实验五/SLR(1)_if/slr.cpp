...
void analyse ( string src )
{
    ...
    for ( int i = 0 ; i < src.length() ; i++ )
    {
        ...
        if (u == 't')
        {
            Lukasiewicz = Lukasiewicz_temp + " P1 jez ";
            Lukasiewicz_temp = "";
        }
        else if(u=='e')
        {
            Lukasiewicz = Lukasiewicz + Lukasiewicz_temp + " P2 jump P1: ";
            Lukasiewicz_temp = "";
        }
        ...
        if ( act.type == 0 )
        {
            ...
            f << get_steps(steps) << '\t'<<'\t' << get_stk(op_stack) << '\t' << '\t' << src.substr(i) << '\t' << '\t' << "移近" 
        ...
        else if ( act.type == 1 )
        {
            ...
            f << get_steps(steps) << '\t' << '\t' <<  get_stk(op_stack) << '\t' << '\t' << src.substr(i) << '\t' << '\t' << get_shift(tt) 
            << '\t' << '\t' << get_stk(st_stack) << '\t' << '\t' << act.out  << '\t' << '\t' << get_steps(x) << endl;
            if ( tt.right == "a" | tt.right == "b" | tt.right == "c" | tt.right == "d" | tt.right == "e")
            {
                Lukasiewicz_temp += tt.right;
            }
            if ( tt.right == "E+T")
            {
                Lukasiewicz_temp += "+";
        ...
        {   
            ...
            Lukasiewicz += Lukasiewicz_temp + " P2: ";
